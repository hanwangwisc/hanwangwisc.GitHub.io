...
- (i) Intuition: Any element in $$B$$ is an upper bound of L, s.t., $$\alpha$$ is a lower bound of B. More formally, because $$ \alpha = sup L \in S $$, $$ \forall　\beta<\alpha $$, $$\beta$$ is not an upper bound of L. Thus $$\beta \notin B$$. Using the contrapositive, for every element $$\beta$$ in B, $$\alpha \leq \beta$$.
