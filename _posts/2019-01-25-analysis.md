...
Proof: Let $$\{G_\alpha\}_\alpha$$ be an open cover of K. Since $$0\in K$$, it is also contained in one of the open sets, called $$G_\0$$. As we know $$G_\0$$ is open, $$\exists \delta>0$$, s.t. $$B(0,delta)	\subset G_\0$$. By Archimedean property, $$\exists N\in \N$$, s.t., $$n\geq N \Rightarrow \frac{1}{n}<\delta$$. i.e., $$\frac{1}{n}\in B(0,delta)	\subset G_\0, \forall n\geq N$$. Suppose $$\frac{1}{n}\in G_{n}$$, for $$n\in \{1,2,3,...,N-1\}$$. So there always exists a finite subcover $$ \cup^{N-1}_{\alpha=1}G_\alpha$$.
