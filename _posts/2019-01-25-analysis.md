...
Claim: $$\forall \epsilon >0, \exists \delta>0, s.t. q\in B_{X}(p,\delta) \Rightarrow f(q)\in B_{X}(f(p),\epsilon)$$.
Pick $$p\in X$$ and $$f(p)\in V$$.  
By openness of V in Y, there exists $$\epsilon$$, s.t. $$B_{Y}(f(p),\epsilon)\subset V$$.
By assumption, we then have an open set $$f^{-1}(V)$$, since $$p\in f^{-1}(V)$$, i.e., there exists $$\delta>0$$, $$B_{X}(p,\delta)\subset f^{-1}(V)$$.

q\in B_{X}(p,\delta) \Rightarrow f(q)\in V \Rightarrow???
