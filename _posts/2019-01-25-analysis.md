...
Theorem. (Dini's thm) Let $$f_{n}: X \rightarrow R$$ be a sequence of continuous functions, with $$f_{n}(x)$$ converges to $$f(x)$$, $$\forall x\in X$$. Assume that:(1) f(x) is continuous on X; (2) the sequence $$\{f_n\}_{n\in N}$$ is monotone increasing, i.e., $$f_{n}(x)<=f_{n+1}(x), \forall x and \forall n$$. Then the sequence $$\{f_n\}_{n\in N}$$ converges uniformly to the function $$f(x)$$.

Proof 1. (Using definitions) 
WTS: $$\forall \epsilon >0, \exists N \in \N, s.t., \forall n\geq \mathbb{N}, \forall x\in X, |f_{n}(x)-f(x)|<\epsilon$$.
Fix $$p\in X$$. By pointwise convergence,  $$\forall \epsilon >0, \exists N_p \in \mathbb{N}, s.t., \forall n\geq N_x, |f_{n}(p)-f(p)|<\epsilon$$. By uniformly continuity (A function defined on compact set is continuous, then it's uniformly continuous.) of $$f_{N_x}$$ and $$f$$, $$\exists \delta_{p}>0 s.t. \forall x\in X and |x-p|<\delta_{p},|f_{N_x}(x)-f_{N_x}(p)|<\epsilon and |f(x)-f(p)|<\epsilon$$.
